...
helm upgrade --install audit-server ./deploy/charts/audit-server -f ./deploy/local.yaml --set image.tag="$version" --set config.githubOauth="$github_oauth" --atomic --timeout 3m0s
